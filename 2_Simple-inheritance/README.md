...
The private header file is necessary so that derived classes, like "mallard.h", can see the full internals of the base class that they're inheriting from, but the rest of the program cannot (at least, not if they're behaving themselves). Unfortunately, the "duckShow()" function has no concept of "Mallard" or "featherColor", so they aren't able to be included in the sentence that gets printed. Additionally, we have no way for the "Mallard" class to add functions to an interface which _further_ derived classes could then inherit. We'll fix these problems in future projects. (The third group of projects, 3a/3b/3c, introduce the concept of polymorphism and allow for functions to "behave" differently when called on different types of objects, though not in a way that's inheritable. The fourth project shows how to create inheritable classes that behave polymorphically, but still require the calling code to explicitly cast an object to different types. The fifth group of projects allows for both, in slightly different ways.)
